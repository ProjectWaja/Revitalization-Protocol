  ...
  return (
    <div className="bg-gray-900 rounded-xl border border-gray-800 p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl font-semibold">Confidential Compute</h2>
          <p className="text-lg text-gray-500 mt-1">Privacy-Preserving Solvency Scoring with Attestation Proofs</p>
        </div>
        {confidentialAddress ? (
          <span className="text-base text-green-400 bg-green-400/10 px-3 py-1 rounded-full">Deployed</span>
        ) : (
          <button
            onClick={handleDeploy}
            disabled={deploying}
            className="px-4 py-2 bg-orange-600 hover:bg-orange-500 disabled:bg-gray-700 rounded text-base font-medium transition-colors"
          >
      ...
      {error && (
        <div className="mb-4 px-3 py-2 bg-red-500/10 border border-red-500/30 rounded-lg text-base text-red-300">
          {error}
        </div>
      )}
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[
              ...
              <div key={s.label} className="bg-gray-800/30 rounded-lg p-4 space-y-2">
          ...
          <div className="flex items-center justify-between bg-gray-800/30 rounded-lg p-4">
            <div className="flex items-center gap-6">
              <div>
                <div className="text-base text-gray-500">Predicted Score</div>
                <div className={`text-2xl font-bold font-mono ${RISK_COLORS[RISK_LEVELS[predictedRisk]]}`}>
                  {predictedScore}
                </div>
              </div>
              <div>
                <div className="text-base text-gray-500">Risk Level</div>
                <div className={`text-lg font-medium ${RISK_COLORS[RISK_LEVELS[predictedRisk]]}`}>
            ...
            <button
              ...
              disabled={computing}
              className="flex items-center gap-2 px-4 py-2 bg-orange-600 hover:bg-orange-500 disabled:bg-gray-700 rounded-lg text-base font-medium transition-colors"
            >
              {computing ? (
                <>
          ...
          {result && (
            <div className="space-y-3">
              <h3 className="text-lg font-medium text-gray-400">Latest Attestation</h3>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <div className="bg-gray-800/50 rounded-lg p-4">
                  <div className="text-gray-400 text-base">On-Chain Score</div>
                  <div className={`text-2xl font-bold font-mono ${RISK_COLORS[RISK_LEVELS[result.riskLevel]]}`}>
                    {result.score}
                  </div>
                </div>
                <div className="bg-gray-800/50 rounded-lg p-4">
                  <div className="text-gray-400 text-base">Risk Level</div>
                  <div className={`text-xl font-bold ${RISK_COLORS[RISK_LEVELS[result.riskLevel]]}`}>
                    {RISK_LEVELS[result.riskLevel]}
                  </div>
                </div>
                <div className="bg-gray-800/50 rounded-lg p-4">
                  <div className="text-gray-400 text-base">Enclave Verified</div>
                  <div className={`text-xl font-bold ${result.enclaveVerified ? 'text-green-400' : 'text-gray-500'}`}>
                    {result.enclaveVerified ? 'Yes' : 'No'}
                  </div>
                </div>
                <div className="bg-gray-800/50 rounded-lg p-4">
                  <div className="text-gray-400 text-base">Computations</div>
                  <div className="text-2xl font-bold font-mono">{result.computationCount}</div>
                </div>
              </div>

              {/* Attestation Hash */}
              <div className="bg-gray-800/30 rounded-lg p-3">
                <div className="text-base text-gray-500 mb-1">Attestation Hash (keccak256 of inputs + score + nonce)</div>
                <div className="font-mono text-base text-orange-400 break-all">{result.attestationHash}</div>
