...
function VerificationBadge({ status }: { status: string }) {
  return (
    <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded text-base font-medium ${STATUS_STYLES[status]}`}>
      <svg className={`w-4 h-4 ${STATUS_ICON[status]}`} viewBox="0 0 20 20" fill="currentColor">
        {status === 'VERIFIED' ? (
  ...
  return (
    <div className="bg-gray-900 rounded-xl border border-gray-800 p-6">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-semibold">Proof of Reserves</h2>
        <span className="text-lg text-gray-400">Chainlink PoR</span>
      </div>

      {/* Project Reserves */}
      <div className="space-y-4 mb-6">
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-medium text-gray-400">Project Reserve Verification</h3>
          <VerificationBadge status={projectStatus} />
        </div>

        <div className="grid grid-cols-2 gap-3">
          <div className="bg-gray-800/50 rounded-lg p-4">
            <div className="text-gray-400 text-base">Claimed Reserves</div>
            <div className="text-2xl font-bold font-mono">${(pr.claimed / 1e6).toFixed(0)}M</div>
          </div>
          <div className="bg-gray-800/50 rounded-lg p-4">
            <div className="text-gray-400 text-base">PoR Verified</div>
            <div className="text-2xl font-bold font-mono">${(pr.porReported / 1e6).toFixed(1)}M</div>
          </div>
        </div>
        ...
        <div className="space-y-1">
          ...
          <div className="text-base text-gray-500 text-right">Min threshold: 80%</div>
        </div>
      </div>
      ...
      <div className="space-y-4 pt-4 border-t border-gray-800">
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-medium text-gray-400">Funding Engine Verification</h3>
          <VerificationBadge status={engineStatus} />
        </div>

        <div className="grid grid-cols-3 gap-3">
          <div className="bg-gray-800/50 rounded-lg p-4">
            <div className="text-gray-400 text-base">On-Chain Balance</div>
            <div className="text-2xl font-bold font-mono">{er.contractBalance.toFixed(2)} ETH</div>
            {ethPrice > 0 && <div className="text-base text-gray-500 font-mono">{formatUsd(er.contractBalance, ethPrice)}</div>}
          </div>
          <div className="bg-gray-800/50 rounded-lg p-4">
            <div className="text-gray-400 text-base">Reported Deposits</div>
            <div className="text-2xl font-bold font-mono">{er.reportedDeposits.toFixed(2)} ETH</div>
            {ethPrice > 0 && <div className="text-base text-gray-500 font-mono">{formatUsd(er.reportedDeposits, ethPrice)}</div>}
          </div>
          <div className="bg-gray-800/50 rounded-lg p-4">
            <div className="text-gray-400 text-base">Coverage</div>
            <div className={`text-2xl font-bold ${coverageRatio >= 100 ? 'text-green-400' : 'text-red-400'}`}>
        ...
        {er.reportedDeposits > 0 && (
          <div className="space-y-1">
            <div className="flex justify-between text-lg">
              ...
              <span className={`font-mono text-base ${coverageRatio >= 100 ? 'text-green-400' : 'text-red-400'}`}>
